...
def frame():
    idx=pd.date_range("2026-08-31 09:15+05:30",periods=20,freq="1h")
    close=pd.Series(range(100,120),index=idx)
    return pd.DataFrame({"open":close-.5,"high":close+1,"low":close-1,"close":close},index=idx)

def test_backtest_uses_selected_account_limit():
    ...
    assert result.planned_risk==6000
