...
def test_dashboard_keeps_approved_theme_and_mobile_contract():
    ...
    assert 'prefers-reduced-motion:reduce' in CSS_COMPACT
    assert 'position:fixed' in CSS_COMPACT
    assert 'bottom:7px' in CSS_COMPACT
...
def test_dashboard_has_clear_visual_hierarchy_tokens():
    for token in ('--surface','--surface-2','--accent','--positive','--negative','--warning','--shadow','--radius'):
        assert token in CSS
    for selector in ('.topbar','.desktop-nav','.page-heading','.hero','.box','.signal-card','.trade-card','.news-item'):
