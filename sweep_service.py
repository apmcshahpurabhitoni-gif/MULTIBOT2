...
class SweepService:
    ...
    def _signal(self,symbol,result,now):
        ...
        tp=entry+2*distance if direction=="BUY" else entry-2*distance; qty=RISK_PER_TRADE_INR/distance; account="nifty" if symbol in NIFTY_SYMBOLS or symbol.endswith('.NS') else "sweep_4h"
    ...
    def _price(self,symbol):
        try:
            data=self.runtime.provider.fetch(symbol,period="2d",interval="1m",validate_hourly=False)
            return None if data.empty else float(data.close.iloc[-1])
        except Exception:return None

    def _fetch(self,symbol,period="30d"):
        if symbol.endswith('.NS'):return self.runtime.fetch_symbol_1h(symbol,period=period)
        return self.runtime.provider.fetch(symbol,period=period,interval="1h",validate_hourly=False)

    def _scan_one(self,symbol,now,send_neutral=True):
        frame=self._fetch(symbol); result=detect_sweep(frame,symbol,now)
        if result is None:return None
        if result.direction=="NEUTRAL":
            if send_neutral:self._send(self._neutral_message(symbol,result,now))
            return result
        if result.direction in ("BULLISH","BEARISH"):
            self._signal(symbol,result,now)
        return result
            ...
            try:
                ...
                for market,times in CHECK_TIMES.items():
                    if (now.hour,now.minute) in times and (market,key) not in seen:
                        seen.add((market,key)); timeframe="4H"; self.last_checks[market]=now.isoformat(); self._send(self._checking_message("Crypto / BTC" if market=="CRYPTO" else "Forex / Gold",timeframe,now.strftime('%H:%M')))
                        symbols=CRYPTO_SYMBOLS if market=="CRYPTO" else FOREX_GOLD_SYMBOLS
                        for symbol in symbols:
                            try:self._scan_one(symbol,now,send_neutral=True)
        ...
        for symbol in NSE_15_SYMBOLS:
            try:
                ...
                if signal.signal=="NEUTRAL":
                    if send:self._send(self._neutral_message(symbol,detect_sweep(frame,symbol,current),current))
                    continue
