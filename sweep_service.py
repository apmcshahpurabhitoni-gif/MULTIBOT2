...
class SweepService:
    ...
    def dispatch(self,symbol,signal,candles_4h,*,current_price,now=None,send=True,account_name=DEFAULT_ACCOUNT):
        ...
        with self._lock:
            ...
            if count>=2:return SweepDispatchResult(symbol,signal,None,None,False,"DUPLICATE_SIGNAL_LIMIT",account_name)
            if count==1:return SweepDispatchResult(symbol,signal,None,None,False,"REMINDER_PENDING",account_name)
            ...
            if signal.signal=="BUY":sl=float(candle.low);distance=entry-sl;tp=entry+2*distance
            else:sl=float(candle.high);distance=sl-entry;tp=entry-2*distance
            if distance<=0:return SweepDispatchResult(symbol,signal,None,None,False,"INVALID_SWEEP_RISK",account_name)
            qty=RISK_PER_TRADE_INR/distance;plan=TradePlan("Sweep V2",signal.signal,signal.timestamp,entry,sl,tp);trade=PaperTrade(plan=plan,account=account_name,quantity=qty);age_m=int(self.gate.age_hours(signal,now=current)*60);age=f"{age_m} min ago" if age_m<60 else f"{age_m//60} hr {age_m%60} min ago"
            ...
            send_message(message,self.telegram_config or TelegramConfig.from_env());self.gate.accept(signal,symbol=symbol,now=current);self.database.record_signal_send(key,current.isoformat(),(current+pd.Timedelta(hours=1)).isoformat(),message.text,{"strategy":"Sweep V2","symbol":symbol,"direction":signal.signal,"timestamp":signal.timestamp.isoformat()});updated=register_trade(account,planned_risk=trade.planned_risk);self.accounts[account_name]=updated;self.database.save_account(account_name,balance=updated.balance,trades_today=updated.trades_today,planned_risk_used=updated.planned_risk_used,reset_date=current.date().isoformat());return SweepDispatchResult(symbol,signal,trade,message,True,"SENT_AND_ACCEPTED",account_name)
    def scan_universe_and_dispatch(self,*,now=None,period="30d",send=True):
        ...
        for symbol in NSE_15_SYMBOLS:
            try:
                ...
                data=self.runtime.provider.fetch(f"{symbol}.NS",period="1d",interval="1m",validate_hourly=False)
                if data.empty:continue
                out.append(self.dispatch(symbol,signal,frame,current_price=float(data.close.iloc[-1]),now=now,send=send))
