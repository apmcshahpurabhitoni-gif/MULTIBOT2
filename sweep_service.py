...
class SweepService:
    ...
    def _reject(self, symbol, signal, reason, account_name):
        return SweepDispatchResult(
            symbol,
            signal,
            None,
            signal_rejection_message(strategy="Sweep V2", symbol=symbol, reason=reason),
            False,
            reason,
            account_name,
        )
    ...
    def _fetch(self, symbol, period="30d"):
        ...
        if normalized.endswith(".NS"):
            return self.runtime.fetch_symbol_1h(normalized, period=period)
        return self.runtime.provider.fetch(normalized, period=period, interval="1h", validate_hourly=False)

    def _price(self, symbol):
        try:
            frame = self.runtime.provider.fetch(symbol, period="2d", interval="1m", validate_hourly=False)
    ...
    def dispatch(self, symbol, signal, candles_1h, *, current_price, now=None, send=True, account_name=DEFAULT_ACCOUNT):
        ...
        if signal.signal not in ("BUY", "SELL"):
            return self._reject(symbol, signal, "NO_DIRECTIONAL_SIGNAL", account_name)
        if not self.gate.is_fresh(signal, now=current):
            return self._reject(symbol, signal, "STALE_SIGNAL", account_name)
        key = self.gate.signal_key(signal, symbol=symbol)
        with self._lock:
            count = self.database.signal_count(key)
            if count >= 2:
                return self._reject(symbol, signal, "DUPLICATE_SIGNAL_LIMIT", account_name)
            if count == 1:
                return self._reject(symbol, signal, "REMINDER_PENDING", account_name)
            account = self.accounts[account_name]
            if not can_open_trade(account):
                return self._reject(symbol, signal, "ACCOUNT_DAILY_LIMIT", account_name)
            if candles_1h is None or candles_1h.empty:
                return self._reject(symbol, signal, "MISSING_SIGNAL_CANDLE", account_name)
            ...
            if plan is None:
                return self._reject(symbol, signal, "NO_DIRECTIONAL_SIGNAL", account_name)
            qty = quantity_for_risk(plan.entry, plan.stop_loss)
            trade = PaperTrade(plan=plan, account=account_name, quantity=qty)
            age = max(0, int((current - signal.timestamp).total_seconds() / 60))
            age_text = f"{age} min ago" if age < 60 else f"{age // 60} hr {age % 60} min ago"
            market = "Crypto" if symbol == "BTC-USD" else "Forex / Gold" if symbol in FOREX_GOLD_SYMBOLS else "NSE"
            asset = {"BTC-USD": "Bitcoin (BTC)", "GC=F": "Gold (XAU/USD)", "^NSEI": "NIFTY 50", "^NSEBANK": "BANK NIFTY"}.get(symbol, symbol.replace(".NS", ""))
    ...
    def scan_symbol(self, symbol, *, period="30d"):
        ...
        frame = self._fetch(symbol, period)
        result = detect_sweep(frame, symbol, pd.Timestamp.now(tz=IST_TIMEZONE))
        ts = result.candle_end if result else (frame.index[-1] if len(frame) else pd.Timestamp.now(tz=IST_TIMEZONE))
        direction = {"BULLISH": "BUY", "BEARISH": "SELL", "NEUTRAL": "NEUTRAL"}.get(result.direction if result else "", "NO_SIGNAL")
        reason = result.direction if result else "NO_SIGNAL"
        return StrategySignal("Sweep V2", direction, ts, reason), frame
        ...
        for symbol in NSE_15_SYMBOLS:
            try:
                ...
                if price is not None:
                    out.append(self.dispatch(symbol, signal, frame, current_price=price, now=current, send=send, account_name=self.DEFAULT_ACCOUNT))
            except Exception:
                continue
        return out
