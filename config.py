...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(timezone, timeframe, SIGNAL_FRESHNESS_HOURS, provider, os.getenv("TELEGRAM_BOT_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("DASHBOARD_API_URL", "/api/dashboard"), os.getenv("BOT_STATE_DB_PATH", "multibot2_state.db"), int(os.getenv("SCAN_INTERVAL_SECONDS", "300")), int(os.getenv("MONITOR_INTERVAL_SECONDS", "20")))
...
def validate_configuration() -> None:
    ...
    if ACCOUNT_SIZE_INR != 100_000 or RISK_PER_TRADE_INR != 2_000: raise ValueError("Locked account/risk values were changed")
    if settings.freshness_hours != 1: raise ValueError("Signal freshness must remain locked at 1 hour")
    if set(ACCOUNT_TRADE_LIMITS) != set(ACCOUNT_NAMES) or tuple(ACCOUNT_TRADE_LIMITS.values()) != (20,5,3,3): raise ValueError("Original independent account limits are required")
    if LEVERAGE != 1.0: raise ValueError("MULTIBOT2 uses 1x / no leverage")
    if settings.scan_interval_seconds < 60 or settings.monitor_interval_seconds < 1: raise ValueError("Runtime intervals are invalid")
