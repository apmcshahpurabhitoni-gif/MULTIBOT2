...
def _explicit(df,starts,duration,now):
    ...
    for start in starts:
        ...
        if not g.empty:rows.append({"timestamp":start,**_ohlc(g)})
...
def detect_sweep(df,symbol,now=None):
    ...
    prev,cur=bars.iloc[-2],bars.iloc[-1];start=pd.Timestamp(bars.index[-1]);end=start+(pd.Timedelta(hours=2) if symbol.endswith('.NS') and start.hour==13 else pd.Timedelta(hours=1) if tf=='1H' else pd.Timedelta(hours=4));hs=float(cur.high)>float(prev.high);ls=float(cur.low)<float(prev.low)
    if not(hs and ls):return None
    close=float(cur.close);direction="BULLISH" if close>float(prev.high) else "BEARISH" if close<float(prev.low) else "NEUTRAL";expected_minute=15 if symbol.endswith('.NS') or symbol in ('^NSEI','^NSEBANK') else 30;expected={9,13} if symbol.endswith('.NS') else {9,10,11,12,13,14} if symbol in ('^NSEI','^NSEBANK') else {1,5,9,13,17,21} if symbol=='BTC-USD' else {2,6,10,14,18,22}
