...
def signal(ts="2026-09-01 12:00:00+05:30",direction="BUY"):
    return StrategySignal("Sweep V2",direction,pd.Timestamp(ts),"TEST")

def frame():
    idx=pd.date_range("2026-09-01 08:00",periods=4,freq="1h",tz="Asia/Kolkata")
    return pd.DataFrame({"open":[100,101,99,102],"high":[105,106,110,112],"low":[95,94,90,89],"close":[101,99,108,111]},index=idx)
...
def test_send_false_does_not_consume_gate(tmp_path):
    s=service(tmp_path); sig=signal(); result=s.dispatch("RELIANCE",sig,frame(),current_price=100,now=pd.Timestamp("2026-09-01 12:30+05:30"),send=False)
...
def test_non_directional_signal_is_silent(tmp_path):
    s=service(tmp_path); result=s.dispatch("RELIANCE",signal(direction="NEUTRAL"),frame(),current_price=100,now=pd.Timestamp("2026-09-01 12:30+05:30"),send=False)
