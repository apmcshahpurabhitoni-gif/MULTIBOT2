...
class StrategySignal:
    strategy:str;signal:SignalType;timestamp:pd.Timestamp;reason:str;entry:float|None=None;atr:float|None=None
...
def _validate_candle(candle,name):
...
def sweep_v2(previous,current,*,timestamp):
    _validate_candle(previous,"Previous");_validate_candle(current,"Current");ts=pd.Timestamp(timestamp)
    if ts.tzinfo is None:raise ValueError("Strategy signal timestamp must be timezone-aware")
    ts=ts.tz_convert(IST_TIMEZONE);ph,pl=float(previous.high),float(previous.low);ch,cl,cc=float(current.high),float(current.low),float(current.close)
...
def sweep_v2_from_frame(candles):
    frame=normalize_index(candles)
    if len(frame)<2:
        ts=frame.index[-1] if len(frame) else pd.Timestamp.now(tz=IST_TIMEZONE);return StrategySignal("Sweep V2","NO_SIGNAL",ts,"INSUFFICIENT_DATA")
    return sweep_v2(frame.iloc[-2],frame.iloc[-1],timestamp=frame.index[-1])
def ema(series,period):return series.ewm(span=period,adjust=False).mean()
def rsi(series,period=14):
    d=series.diff();g=d.clip(lower=0).rolling(period).mean();l=(-d.clip(upper=0)).rolling(period).mean();rs=g/l.replace(0,pd.NA);out=(100-100/(1+rs)).astype(float);out=out.where(l.ne(0),100.0);return out.fillna(50.0)
...
def trendpulse_from_frames(candles_1h,candles_4h=None,*,completed_only=True):
    ...
    ema20=ema(one.close,20);rsi14=rsi(one.close,14);atr1=atr(one,14);ml,ms=macd(one.close);ema50=ema(four.close,50);atr4=atr(four,14);ht=eligible.index[-1]
    vals=[one.close.iloc[i],ema20.iloc[i],rsi14.iloc[i],atr1.iloc[i],ml.iloc[i-1],ml.iloc[i],ms.iloc[i-1],ms.iloc[i],four.close.loc[ht],ema50.loc[ht],atr4.loc[ht]]
    if any(pd.isna(v) for v in vals):return StrategySignal("TrendPulse","NO_SIGNAL",ts,"INDICATOR_DATA_UNAVAILABLE")
    hclose=float(four.close.loc[ht]);hat=float(atr4.loc[ht]);he=float(ema50.loc[ht])
    if hclose<=0:return StrategySignal("TrendPulse","NO_SIGNAL",ts,"INVALID_HTF_CLOSE")
    if hat/hclose*100<.2:return StrategySignal("TrendPulse","NO_SIGNAL",ts,"ATR_PERCENT_BELOW_0_2")
    bull=hclose>he and ml.iloc[i-1]<=ms.iloc[i-1] and ml.iloc[i]>ms.iloc[i] and 50<rsi14.iloc[i]<80 and one.close.iloc[i]>ema20.iloc[i]
    bear=hclose<he and ml.iloc[i-1]>=ms.iloc[i-1] and ml.iloc[i]<ms.iloc[i] and 20<rsi14.iloc[i]<50 and one.close.iloc[i]<ema20.iloc[i]
    if bull:return StrategySignal("TrendPulse","BUY",ts,"HTF_BULLISH_MACD_RSI_EMA_ALIGNMENT",float(one.close.iloc[i]),float(atr1.iloc[i]))
    if bear:return StrategySignal("TrendPulse","SELL",ts,"HTF_BEARISH_MACD_RSI_EMA_ALIGNMENT",float(one.close.iloc[i]),float(atr1.iloc[i]))
    return StrategySignal("TrendPulse","NO_SIGNAL",ts,"NO_APPROVED_ALIGNMENT")
def calc_sl_tp(signal):
    ...
    e=float(signal.entry);r=float(signal.atr)*1.5;return (e-r,e+r*2) if signal.signal=="BUY" else (e+r,e-r*2)
