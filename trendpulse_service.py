...
class TrendPulseDispatchResult:
    symbol:str;signal:object;scan:TrendPulseScanResult;trade:PaperTrade|None;message:TelegramMessage|None;sent:bool;reason:str;account:str="nifty"
class TrendPulseService:
    ...
    def dispatch_result(self,scan,*,now=None,send=True,account_name=DEFAULT_ACCOUNT):
        ...
        with self._lock:
            ...
            sl,tp=calc_sl_tp(signal);entry=float(signal.entry);plan=TradePlan(signal.strategy,signal.signal,signal.timestamp,entry,sl,tp);qty=RISK_PER_TRADE_INR/plan.risk_per_unit;trade=PaperTrade(plan=plan,account=account_name,quantity=qty);age_m=int(self.runtime.gate.age_hours(signal,now=current)*60);age=f"{age_m} min ago" if age_m<60 else f"{age_m//60} hr {age_m%60} min ago";message=render_signal_message(signal,symbol=f"{scan.symbol}.NS",asset=scan.symbol,market="NSE",timeframe="1H",entry=entry,stop_loss=sl,take_profit=tp,quantity=qty,risk=trade.planned_risk,account=account_name,freshness="FRESH",age_str=age)
            if not send:return TrendPulseDispatchResult(scan.symbol,signal,scan,trade,message,False,"READY_TO_SEND",account_name)
            send_message(message,self._config());self.database.record_signal_send(key,current.isoformat(),(current+pd.Timedelta(hours=1)).isoformat(),message.text,{"message_type":message.message_type,"strategy":signal.strategy,"symbol":scan.symbol,"direction":signal.signal,"timestamp":signal.timestamp.isoformat()});updated=register_trade(account,planned_risk=trade.planned_risk);self.accounts[account_name]=updated;self.database.save_account(account_name,balance=updated.balance,trades_today=updated.trades_today,planned_risk_used=updated.planned_risk_used,reset_date=current.date().isoformat());self.runtime.gate.accept(signal,symbol=scan.symbol,now=current);return TrendPulseDispatchResult(scan.symbol,signal,scan,trade,message,True,"SENT_AND_ACCEPTED",account_name)
