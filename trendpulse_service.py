...
class TrendPulseService:
    ...
    def _reject(self, symbol, signal, reason, *, send=True, account_name=DEFAULT_ACCOUNT, detail=""):
        # Rejections are internal dispatch diagnostics. They must never become
        # Telegram spam during automated universe scans. Directional accepted
        # signals are the only TrendPulse messages sent to Telegram.
    ...
    def dispatch_result(self, scan, *, now=None, send=True, account_name=DEFAULT_ACCOUNT):
        ...
        with self._lock:
            ...
            self.database.record_signal_send(key, current.isoformat(), (current + pd.Timedelta(hours=1)).isoformat(), message.text, {"message_type": message.message_type, "strategy": signal.strategy, "symbol": scan.symbol, "direction": signal.signal, "timestamp": signal.timestamp.isoformat()})
