"""Complete TrendPulse dispatch: freshness -> duplicate -> risk -> persistence -> Telegram."""
...
class TrendPulseService:
    ...
    def dispatch_result(self,scan,*,now=None,send=True,account_name=DEFAULT_ACCOUNT):
        ...
        with self._lock:
            ...
            age_m=int(self.runtime.gate.age_hours(signal,now=current)*60); age=f"{age_m} min ago" if age_m<60 else f"{age_m//60} hr {age_m%60} min ago"
            ...
            if not self.runtime.gate.can_send(signal,symbol=scan.symbol,now=current):return TrendPulseDispatchResult(scan.symbol,signal,scan,None,None,False,"DUPLICATE_SIGNAL_LIMIT",account_name)
            send_message(message,self._config()); accepted=self.runtime.gate.accept(signal,symbol=scan.symbol,now=current)
            if not accepted:raise RuntimeError("Signal acceptance failed after Telegram delivery")
            self.database.record_signal_send(key,current.isoformat(),(current+pd.Timedelta(hours=1)).isoformat(),message.text,{"strategy":signal.strategy,"symbol":scan.symbol,"direction":signal.signal,"timestamp":signal.timestamp.isoformat()})
